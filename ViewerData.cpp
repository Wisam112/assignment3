...
//Assignment 3
/*Eigen::Matrix4f igl::opengl::ViewerData::MakeTransWithoutTranslate2()
{
	return nul
}*/

Eigen::Matrix4f igl::opengl::ViewerData::MakeTrans2()
{
	if (Parent == nullptr) {
		return Translate1.matrix() * Theta.matrix() * Phi.matrix() * Psi.matrix() *IK.matrix()* Translate3.matrix()* Scale.matrix();
	}
	else {
		return Parent->MakeTrans2() * Theta.matrix() * Phi.matrix() * Psi.matrix()*IK.matrix()  *Translate1.matrix()* Scale.matrix();
	}
}
Eigen::Matrix4f igl::opengl::ViewerData::MakeTrans()
{
	if (Parent == nullptr)
	{
		return MakeTrans2() * Translate2.matrix();
	}
	else
		return MakeTrans2() * Translate2.matrix();
...
void igl::opengl::ViewerData::MyTranslate(Eigen::Vector3f amt)
{
	if (Parent == nullptr) {
		Translate1.pretranslate(amt);
	}
	else {
		Parent->MyTranslate(amt);
	}
}
...
void igl::opengl::ViewerData::drawAxis(int t) {
	...
	E_points1<< 
		((m(0) + M(0)) / 2 ) +1.6 , M(1)   , (((M(2) + m(2))) / 2) ,
		(((m(0) + M(0))) / 2) - 1.6 , M(1) , (((M(2) + m(2))) / 2)  ;
	Eigen::MatrixXd E_points2(2, 3);
	E_points2 <<
		((m(0) + M(0)) / 2) , M(1) , (((M(2) + m(2))) / 2)+1.8,
		(((m(0) + M(0))) / 2) , M(1) , (((M(2) + m(2))) / 2) -1.8;
	Eigen::MatrixXd E_points3(2, 3);
	E_points3 << ((m(0) + M(0)) / 2), M(1)-1.6, (((M(2) + m(2))) / 2) ,
		(((m(0) + M(0))) / 2), M(1)+1.8, (((M(2) + m(2))) / 2) ;

	
	add_points(V_points, Eigen::RowVector3d(0, 0, 1));
	//add_points(E_points1, Eigen::RowVector3d(1, 0, 1));
	add_edges(E_points1.row(0), E_points1.row(1), Eigen::RowVector3d(1, 0, 0));
	//add_points(E_points2, Eigen::RowVector3d(0, 0, 1));
	add_edges(E_points2.row(0), E_points2.row(1), Eigen::RowVector3d(0, 0, 1));
	add_edges(E_points3.row(0), E_points3.row(1), Eigen::RowVector3d(0, 1, 0));
	...
	V_box <<
		(m(0) + M(0)) / 2, (m(1) + m(1)) / 2, (m(2) + m(2)) / 2 + 2,
		//m(0), m(1), m(2),
		(m(0) + M(0)) / 2, (m(1) + m(1)) / 2, (M(2) + M(2)) / 2 - 2,
		//M(0), m(1), m(2),
		(m(0) + m(0)) / 2 + 2, (m(1) + m(1)) / 2, (M(2) + m(2)) / 2,
		//m(0), m(1), M(2),
		(M(0) + M(0)) / 2 - 2, (m(1) + m(1)) / 2, (M(2) + m(2)) / 2,
		//M(0), m(1), M(2);

		((M(0) + m(0)) / 2) + ((M(0) + m(0)) / 2) / 2, ((M(1) + M(1)) / 2) + ((M(1) + M(1)) / 2) / 2, ((m(2) + m(2)) / 2) + ((M(2) + M(2)) / 2) / 2, //Top

		(((m(0) + M(0)) / 2 + m(0) + M(0)) / 2) / 2, ((m(1) + m(1)) / 2 + (m(1) + m(1)) / 2) / 2, ((m(2) + m(2)) / 2 + (M(2) + M(2)) / 2) / 2; //Bottom
	...
	V_points <<
		(((m(0) + M(0)) / 2 + m(0) + M(0)) / 2) / 2, ((m(1) + m(1)) / 2 + (m(1) + m(1)) / 2) / 2, ((m(2) + m(2)) / 2 + (M(2) + M(2)) / 2) / 2;
