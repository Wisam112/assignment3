...
void Renderer::MouseProcessing(int button)
{
	...
	{
		if (world == false) {
			scn->data().MyTranslate(Eigen::Vector3f(-xrel / 300.0f, 0, 0));
			scn->data().MyTranslate(Eigen::Vector3f(0, yrel / 300.0f, 0));

		}
		else{
			int savedIndex = scn->selected_data_index;
			    scn->MyTranslate(Eigen::Vector3f(-xrel / 300.0f, 0, 0));
				scn->MyTranslate(Eigen::Vector3f(0, yrel / 300.0f, 0));
	...
	{
		if (world == false) {
			scn->data().MyRotate(Eigen::Vector3f(0, 1, 0), xrel / 220.0f);
			scn->data().MyRotate(Eigen::Vector3f(1, 0, 0), yrel / 220.0f);
		
		}
		else {
			int savedIndex = scn->selected_data_index;
				scn->MyRotate(Eigen::Vector3f(0, 1, 0), xrel / 220.0f);
				scn->MyRotate(Eigen::Vector3f(1, 0, 0), yrel / 220.0f);
