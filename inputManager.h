...
static void glfw_mouse_scroll(GLFWwindow* window, double x, double y)
{
	Renderer* rndr = (Renderer*)glfwGetWindowUserPointer(window);
	igl::opengl::glfw::Viewer* scn = rndr->GetScene();
	int savedIndex = scn->selected_data_index;
	if (rndr->world == false) {
		if (scn->selected_data_index > 0)
		{
			scn->selected_data_index = 1;
			rndr->GetScene()->data().MyTranslate(Eigen::Vector3f(0,0,  y * 0.1));
		}
		else
			rndr->GetScene()->data().MyTranslate(Eigen::Vector3f(0, 0,  y * 0.1));
			...
			rndr->GetScene()->MyTranslate(Eigen::Vector3f(0, 0,  y * 0.1));
			
		
	}
	scn->selected_data_index = savedIndex;
}
...
static void glfw_key_callback(GLFWwindow* window, int key, int scancode, int action, int modifier)
{
	...
	if (key == GLFW_KEY_ESCAPE && action == GLFW_PRESS)
		...
		case 't':
		{ 
			int save_ind = scn->selected_data_index;
			for (int i = 1; i <= scn->arms; i++)
			{
				scn->selected_data_index = i;
				std::cout <<" tip for arm  "<<i << std::endl;
				std::cout << scn->data().getTop() << std::endl;
			}
			scn->selected_data_index = save_ind;
		...
		case 'p':
		{
			if (rndr->world) 
			{
				...
				std::cout << scn->Theta.matrix() << std::endl;
				std::cout << "Phi : " << std::endl;
				std::cout << scn->Phi.matrix() << std::endl;
				std::cout << "Psi : " << std::endl;
				std::cout << scn->Psi.matrix ()<< std::endl;
				
			}
			else
			{
				std::cout << "Theta : " << std::endl;
				std::cout << scn->data().Theta.matrix() << std::endl;
				std::cout << "Phi : " << std::endl;
				std::cout << scn->data().Phi.matrix() << std::endl;
				std::cout << "Psi : " << std::endl;
				std::cout << scn->data().Psi.matrix ()<< std::endl;
			}
			break;
		}
		case 'd':
		case 'D':
		{
			int saved_index = scn->selected_data_index;
			scn->selected_data_index = 0;
			std::cout << " destnation  : " << std::endl;
			std::cout << scn->data().getCeneter() << std::endl;
			scn->selected_data_index = saved_index;
			break;
		}
		case 'R':
		case 'r':
		{
			int save_ind = scn->selected_data_index;
			for (int i = 0; i <= scn->arms; i++)
			{
				
				scn->selected_data_index = i;
				scn->data().reset_matrix();
				
			}
			//scn->reset_matrix();
			scn->init_ik(scn->arms);
			scn->selected_data_index = save_ind;
			break;
		}
		case '1':
